<template>
	<div style="margin: 10px;" class="jwt-div">
		<el-container>
			<el-header><span style="font-weight: 900">URL Encoder and Decoder</span></el-header>
			<el-card>
...
export default defineComponent({
	...
	setup() {
		const state = reactive({
			loading: false,
			inputContent: '/api/user/update%3Fkey%3D%7B%22idNumber%22%3A%22ID231000011%22%2C%22name%22%3A%22Jacky%22%2C%22role%22%3A%22Admin%22%7D',
			outputContent: '',
		...
		const convert = () => {
			...
			if (state.convertionMode === 'Decode') {
				state.outputContent = decodeURIComponent(state.inputContent)
			} else {
				state.outputContent = encodeURIComponent(state.inputContent);
			}
		};
		return {
			...toRefs(state),
